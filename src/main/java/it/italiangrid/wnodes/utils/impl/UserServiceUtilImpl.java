...
public class UserServiceUtilImpl implements UserServiceUtil {
	...
	public boolean createSshKey() {
		String key = System.getProperty("java.io.tmpdir")+"/users/"+userId+"/id_rsa";
		log.info("Directory = " + key);
		
		File keyFile = new File(key);
		
		if(!keyFile.exists()){
			String[] cmd = {"/usr/bin/ssh-keygen", "-t", "rsa", "-N", "", "-f", key};
			try {
	...
	private void printOutput(InputStream stdout, InputStream stderr) throws IOException {
